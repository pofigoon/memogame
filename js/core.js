...
var _self; var _config; var _data; var _canvas;
...
{FRONT}\
           ...
           if (!$(this).hasClass("flipped")) {
               if (!_data['currentItem']) {
                   ...
                   if (_data['currentItem'][0] == $(this).attr("data-id") && this.id != _data['currentItem'][1]) {
                       ...
                       // close them both
                    setTimeout(function () {
                        $(".axgame .axplay .card:not(.flipped)").removeClass("flip");
},_config.delay);
                   }
 ...
 return 0.5 - Math.random();
        ...
        $.each(cardsArray, function (i, e) {
            cnt++;
            _canvas.find(".axplay .container").append(_tpl.card.replace(/{ID}/g,e[1]).replace(/{BACK}/g, e[0])
.replace(/{FRONT}/g, (_config._debug ? e[1] : ""))
.replace("{CARD_ID}", cnt));
        });
};
